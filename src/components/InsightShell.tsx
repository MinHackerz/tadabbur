...
export default function InsightShell({route,chapterId="1"}:{route:string;chapterId?:string}){
  ...
  return (
    <PageShell active={route as any} loggedIn={li} userEmail={(d.userInfo?.data as {email?: string} | null)?.email ?? null} variant={isReader?"reader":route==="home"?"home":isFeature?"feature":"default"} readerChapterId={chapterId}>
