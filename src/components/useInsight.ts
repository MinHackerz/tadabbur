...
export function useInsight(chapterId:string,route:string) {
  ...
  function repl<T>(k:"bookmarks"|"collections"|"notes",fn:(i:T[])=>T[]){if(!data)return;mutate({...data,[k]:{...data[k],items:fn(data[k].items as T[])}},false)}
  ...
  async function submitGoalPayload(payload:Record<string,unknown>){if(!data){push("Not loaded.",false);return}try{const r=await mutReq("/api/goals","POST",{payload});setGoalTxt(JSON.stringify(payload,null,2));push(r.message??"Saved.",true);const goalData=(r.data&&typeof r.data==="object"&&Object.keys(r.data as object).length>0)?r.data as Record<string,unknown>:payload;startTransition(()=>{mutate({...data,goals:{...data.goals,data:goalData}},false)});await mutate()}catch(e){push((e as MutR).message??"Failed.",false);await mutate()}}
