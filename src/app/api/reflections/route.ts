...
export async function POST(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[DEBUG] Reflection payload:', payload);
  }
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[DEBUG] Parsed reflection:', { body: body.substring(0, 50), verseKey });
  }
  ...
  if (result.error) {
    if (process.env.NODE_ENV === 'development') {
      console.log('[DEBUG] Reflection error:', result.error);
    }
    return mutationError(sessionContext, {
